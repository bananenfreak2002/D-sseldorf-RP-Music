...
module.exports = {
  name: "loop",
  description: "Schaltet den Musik-Loop-Modus ein oder aus.",
  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription('**Loop es! So spielt die gleiche Musik immer weiter. **')
      ...
      interaction?.reply({ embeds: [embed], components: [button], fetchReply: true }).then(async Message => {
        ...
        col.on('collect', async (button) => {
          ...
          if (!queue1 || !queue1.playing) {
            await interaction?.editReply({ content: '⚠️ Es wird keine Musik gespielt!!', ephemeral: true }).catch(e => { })
            await button?.deferUpdate().catch(e => {})
          }
          switch (button.customId) {
            case 'queue':
              const success = queue.setRepeatMode(2);
              interaction?.editReply({ content: `✅ Schleifenwarteschlange!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'nowplaying':
              const success2 = queue.setRepeatMode(1);
              interaction?.editReply({ content: `✅ Looping aktiviert!!` }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
            case 'close':
              if (queue.repeatMode === 0) {
                await button?.deferUpdate().catch(e => {})
                return interaction?.editReply({ content: '⚠️ Looping bereits ausgeschaltet!!', ephemeral: true }).catch(e => { })
              }
              const success4 = queue.setRepeatMode(0);
              interaction?.editReply({ content: '▶️ Looping ausgeschaltet' }).catch(e => { })
              await button?.deferUpdate().catch(e => {})
              break
          }
        })
        col.on('end', async (button) => {
          ...
          const embed = new EmbedBuilder()
            .setColor('#fc5203')
            .setTitle('▶️ Looping ausgeschaltet!!')
            .setTimestamp()
